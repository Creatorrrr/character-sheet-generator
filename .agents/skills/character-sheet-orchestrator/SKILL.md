...
- Use `assets/master-sheet-template.png` as the default master-sheet layout reference unless the user requests another layout. Follow its panel hierarchy and spacing, not its mannequin placeholders or exact English copy.
- Prefer text-free image generation for the art draft, then programmatic text overlay for the final sheet. Use image-model text insertion only as a fallback.
...
5. Generate the text-free draft sheet.
   Focus on character consistency, layout, sections, expressions, turnaround, detail panels, and whitespace. Allow short section labels only; avoid dense body text.

6. Review the draft.
   Check identity consistency, missing panels, expression variety, text-space adequacy, outfit/detail fidelity, and whether regeneration is needed.
