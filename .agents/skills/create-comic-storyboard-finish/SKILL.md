...
description: Use when an approved comic storyboard page needs tone, color, lettering, or final polish from a parent-inspected storyboard_conti_sketch_ink image while preserving its spatial validation description.
---
...
- Required prior-stage reference from `01_storyboard_conti_sketch_ink/` or a recorded/imported conti/sketch/ink path.
- Conti/sketch/ink `*_desc.md` reference; keep it as the spatial validation overlay for the approved comic page design.
- `Required image attachments` / `Prior page continuity references`, including the current page's conti/sketch/ink image and previous pages' inspected finish images when the pack runner provides them.
...
- Preserve the inspected `storyboard_conti_sketch_ink` page structure.
- Preserve the stage-1 positions, vectors, occlusion, line of sight, visibility, location anchors, movement paths, cause-effect logic, and temporal state fields recorded in the `*_desc.md` without redesigning the page layout around them.
- Do not change page layout, panel count, panel shapes, reading order, gutters, negative space, text placement or required text absence, visual emphasis, light clean-line structure, comic effect lines, character/object placement, motion direction, or action logic.
- Add tone, color if requested, lighting, shadows, texture, cleanup, and final polish.
- Preserve the approved focal hierarchy and avoid flattening or hiding the stage-1 line rhythm.
...
- Keep characters, props, setting, landmarks, profile details, and page-layout references consistent with the approved plan, allowed sources, and inspected conti/sketch/ink image.
- Preserve the inspected `storyboard_conti_sketch_ink` eye, face, hand, limb, silhouette, body proportion, and posture structure. Tone/color/final cleanup must not turn a two-eyed character into a one-eyed appearance or change species/body type unless explicitly approved.
...
- Use `character_locks`, `must_match`, source references, page/panel notes, and the parent-inspected conti/sketch/ink image as the source of truth for approved anatomy or non-human exceptions.
...
Do not pass a finish output that changes a passed two-eye, face, hand, limb, silhouette, body-proportion, or posture structure from the conti/sketch/ink stage unless that exception is explicitly approved in the plan or source.
