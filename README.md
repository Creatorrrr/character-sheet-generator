...
- 층/고도, 계단, 난간, 복층 공간, 방/복도/건물처럼 3D 관계가 중요한 컷은 `scene_3d` 검수 모드를 사용할 수 있습니다. 이 모드는 페이지 구성 계획에서 추출한 provisional validation model이며, 생성 reference를 자동 첨부하거나 컷 구도를 대체하지 않습니다.
...
- `spatial_contract`가 있는 계획은 `spatial-preview`로 정적 HTML 검수 화면을 생성해 컷별 위치, 방향 벡터, 이동 경로, 가림·차단/시선 관계, 랜드마크/상태 유지 조건과 `spatial-check` 이슈를 이미지 생성 전에 확인할 수 있습니다. `scene_3d`가 있으면 level/camera/snapshot/transition/lock 데이터와 dependency-free 3D projected preview도 함께 표시합니다. 이 검수는 승인된 만화 페이지 설계를 확인하는 단계이며, 페이지/컷 설계를 대체하지 않습니다.
