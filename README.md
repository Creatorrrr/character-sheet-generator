...
줄거리/시나리오, 장면 메모, 대본, 씬 브리프, 샷 리스트를 바탕으로 기본 2-4컷의 여유 있는 영화적 호흡과 실험적 자유형 컷 구성을 가진 만화책 페이지 계획을 만들고, 승인 후 2단계 이미지 생성을 진행합니다. 각 단계는 최대 4페이지씩 서브에이전트 병렬 생성, 서브에이전트 1차 검수, 부모 세션 최종 검수, 단계 마무리 검수를 거칩니다.

진행 단계:

1. 콘티/스케치/펜선 `storyboard_sketch_ink`
2. 톤/채색/마무리 `finish`

기본 정책:

- 승인 전에는 이미지를 생성하지 않습니다.
- 기본 페이지는 2-4컷의 영화적 호흡을 사용하고, 5컷 이상은 몽타주/개그 타이밍/빠른 액션 연쇄처럼 명확한 이유가 있을 때만 사용합니다.
...
- 다음 단계는 이전 단계의 모든 페이지가 부모 검수와 단계 마무리 검수를 통과한 뒤에만 진행합니다.
...
기본은 2-4컷의 여유 있는 영화적 호흡으로 하고, 사선/비대칭/오픈 컷 같은 자유로운 컷 구성을 활용해줘.
먼저 페이지 목록과 컷 구성, 각색 대사/SFX, 공간/동선 검수 기준을 보고하고, 승인 후 콘티/스케치/펜선 -> 톤/채색/마무리 2단계로 생성해줘.
```
