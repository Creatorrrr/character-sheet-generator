...
| 줄거리/시나리오를 여러 컷이 들어간 만화책 페이지로 단계별 생성하기 | `$create-comic-storyboard-pack` |
...
줄거리/시나리오, 장면 메모, 대본, 씬 브리프, 샷 리스트를 바탕으로 여러 컷이 들어간 만화책 페이지 계획을 만들고, 승인 후 2단계 이미지 생성을 진행합니다. 각 단계는 최대 4페이지씩 서브에이전트 병렬 생성, 서브에이전트 1차 검수, 부모 세션 최종 검수, 단계 마무리 검수를 거칩니다.
...
먼저 페이지 목록과 컷 구성, 각색 대사/SFX, 공간/동선 검수 기준을 보고하고, 승인 후 콘티 -> 스케치/펜선 -> 톤/채색/마무리 순서로 생성해줘.
```
