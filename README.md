...
줄거리/시나리오, 장면 메모, 대본, 씬 브리프, 샷 리스트를 바탕으로 여러 컷이 들어간 만화책 페이지 계획을 만들고, 승인 후 2단계 이미지 생성을 진행합니다. 각 단계는 최대 4페이지씩 서브에이전트 병렬 생성, 서브에이전트 1차 검수, 부모 세션 최종 검수를 거칩니다.
...
- 모든 페이지의 `storyboard_sketch_ink` 부모 검수 통과 전에는 `finish`를 진행하지 않습니다.
