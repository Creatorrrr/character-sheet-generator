...
- `spatial_contract`는 page/composition driver가 아니라 validation overlay입니다. 방향선/가림·차단/벡터/랜드마크/상태 유지 오류를 잡기 위한 것이며, 공간 관계를 잘 보이게 하려고 컷을 공간 도식처럼 설계하지 않습니다.
- 층/고도, 계단, 난간, 복층 공간, 방/복도/건물처럼 3D 관계가 중요한 컷은 특별한 사유가 없으면 `scene_3d` 검수 모드를 기본으로 사용합니다. `panel_screen_2d`는 그래픽/UI 컷, 상징 컷, 감정 클로즈업, 텍스트/SFX 배치 중심 컷, 또는 3D 추정이 가짜 제약을 만들 때만 예외 사유를 기록하고 사용합니다. `scene_3d`는 페이지 구성 계획에서 추출한 provisional validation model이며, 생성 reference를 자동 첨부하거나 컷 구도를 대체하지 않습니다.
- `scene_3d`는 연결된 공간마다 하나의 공유 모델을 두고, 컷별 `panel_snapshots`와 컷 사이 `transitions`로 위치/상태/장소 변화를 기록합니다. 같은 공간을 컷마다 별도 3D로 만들지 않습니다.
- `scene_3d` 검수는 hard/soft/inferred를 구분합니다. hard invariant(예: A는 1층, B는 2층, 문은 원인 없이 열리지 않음)는 rerun 기준이고, soft/inferred geometry(예: 카메라 FOV, 가구의 미세 위치)는 승인된 콘티와 hard invariant를 해치지 않을 때 `reconciled`로 보정 기록할 수 있습니다.
...
- `spatial_contract`가 있는 계획은 `spatial-preview`로 정적 HTML 검수 화면을 생성해 컷별 위치, 방향 벡터, 이동 경로, 가림·차단/시선 관계, 랜드마크/상태 유지 조건과 `spatial-check` 이슈를 이미지 생성 전에 확인할 수 있습니다. `scene_3d`가 있으면 level/camera/snapshot/transition/lock 데이터를 외부 의존성 없는 orbit-capable canvas preview로 함께 표시하며, 기본 isometric view, 드래그 회전, shift/middle 드래그 pan, wheel zoom, top/front/side/iso/camera preset, 같은 `scene_id` preview의 선택적 sync를 지원합니다. 기본 라벨은 짧은 key label이고, `key/all/off` label mode, 라벨 충돌 회피/leader line, z-level rail, 페이지별 status strip, legend/constraint/annotation hover highlight를 제공합니다. 주요 건물/차량/벽/슬랩/엄폐물은 preview-only wireframe box로, 캐릭터는 골격과 방향을 읽을 수 있는 stick-mannequin으로, 건축물은 바닥/벽/기둥/천장을 구분하는 `building_shell`/parts로, 단순 오브젝트는 box/cylinder/flat plane으로 표시할 수 있습니다. 궤적/엄폐/차단 관계와 간단 메모는 점선·화살표·annotation overlay로 표시하며, 각 level plane은 전체 전장에 깔지 않고 해당 level의 active/referenced entity footprint 주변으로 제한합니다. Actors/Obstacles/Landmarks/Relations/Vectors/Annotations/Camera/Ghost panels/Levels 필터로 복잡도를 조절합니다. 선택 필드인 `preview_geometry`는 fixed entity, `spatial_contract.entities[]`, `panel_snapshots[].entities[]`에 `shape`, `size`, `yaw_degrees`, `anchor`, `style`, `preview_label`, `parts[]`를 지정하는 검수용 보조 정보이고, 지원 shape는 `humanoid_mannequin`, `building_shell`, `wall`, `pillar`, `floor_slab`, `ceiling_slab`, `box`, `cylinder`, `flat_plane`입니다. `spatial_contract.annotations[]`는 `id`, `panel`, `text`, `entities`, `line_from`, `line_to`, optional `position`으로 관계 메모를 표시합니다. 이 정보는 `spatial-check` hard validation과 이미지 생성 reference에 영향을 주지 않습니다. `scene_3d` 페이지에서는 raw x/y world coordinate가 의미 있는 panel-screen projection이 아니므로 기존 2D SVG 보조 뷰를 렌더링하지 않습니다. 이는 WebGL/Three.js 렌더러나 generation reference가 아니라 검수용 lightweight 3D projection입니다. 이 검수는 승인된 만화 페이지 설계를 확인하는 단계이며, 페이지/컷 설계를 대체하지 않습니다.
- `spatial-render-manifest`는 Codex Playwright/Browser가 PNG screenshot을 저장할 대상 목록을 생성합니다. 모든 `scene_3d` panel은 camera view를 기본으로 하고, 층/고도/계단/난간/발코니는 iso/side, 엄폐/시선/trajectory는 top view를 추가합니다. manifest의 PNG는 승인 전 계획 검수와 생성 이미지 부모 검수에서 JSON과 함께 읽는 보조 자료이며, runner 자체는 Playwright 의존성을 추가하지 않습니다.
