...
줄거리/시나리오, 장면 메모, 대본, 씬 브리프, 샷 리스트를 바탕으로 기본 3-5컷 권장의 영화적 호흡과 실험적 자유형 컷 구성을 가진 만화책 페이지 계획을 만들고, 승인 후 2단계 이미지 생성을 진행합니다. 각 단계는 최대 4페이지씩 서브에이전트 병렬 생성, 서브에이전트 1차 검수, 부모 세션 최종 검수, 단계 마무리 검수를 거칩니다.
