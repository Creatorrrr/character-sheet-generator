...
class CharacterCloseupPackRunnerTest(unittest.TestCase):
    ...
    def test_next_batch_blocks_dependents_until_anchor_passes_then_reserves_four(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("06_eye_detail.png", second_batch.stdout)
    ...
    def test_full_preset_honors_full_body_front_dependency_in_batch(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = root / "source.png"
            source.write_bytes(b"fake image")
            ...
            for item in state["items"]:
                if item["output"] == "01_face_front.png":
                    item["status"] = "inspected_pass"
                elif item["output"] in {"17_full_body_front.png", "18_full_body_side_back_pair.png", "19_idle_pose.png"}:
                    item["status"] = "pending"
                else:
                    item["status"] = "inspected_pass"
            state_path.write_text(json.dumps(state, indent=2) + "\n")
            ...
            batch = run_cli("next-batch", "--run-dir", str(run_dir), "--limit", "4", cwd=root)
            self.assertIn("17_full_body_front.png", batch.stdout)
            self.assertNotIn("18_full_body_side_back_pair.png", batch.stdout)
            self.assertNotIn("19_idle_pose.png", batch.stdout)
